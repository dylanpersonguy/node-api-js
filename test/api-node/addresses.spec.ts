...
it('balance confirmations', async () => {
  ...
  const balanceConfirmations = await api.addresses.fetchBalanceConfirmations(
    address,
    confirmations,
  );
...
it('effective balance confirmations', async () => {
  const { address } = STATE.ACCOUNTS.SIMPLE;
  const confirmations = 3;
  const balanceConfirmations = await api.addresses.fetchEffectiveBalanceConfirmations(
    address,
    confirmations,
  );
