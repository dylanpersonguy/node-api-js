...
it('details array, longs as string', async () => {
  const info = await api.assets.fetchAssetsDetails(
    [STATE.ASSETS.BTC.id, STATE.ASSETS.ETH.id],
...
it('details string', async () => {
  const info: TAssetDetails<TLong> = (await api.assets.fetchDetails(
    STATE.ASSETS.BTC.id as any,
  )) as any;
...
it('Assets address limit, long as string', async () => {
  const info = await api.assets.fetchAssetsAddressLimit(
    STATE.ACCOUNTS.SIMPLE.address,
    999,
    largeNumbeConvertHeader,
  );
  expect(info).toBeInstanceOf(Array);
...
it('Asset balance, long as string', async () => {
  const info = await api.assets.fetchAssetsBalance(
    STATE.ACCOUNTS.SIMPLE.address,
    largeNumbeConvertHeader,
  );
  expect(info.address).toBe(STATE.ACCOUNTS.SIMPLE.address);
...
it('Balance address assetId', async () => {
  const info = await api.assets.fetchBalanceAddressAssetId(
    STATE.ACCOUNTS.SIMPLE.address,
    STATE.ASSETS.BTC.id,
  );
  expect(info.address).toBe(STATE.ACCOUNTS.SIMPLE.address);
