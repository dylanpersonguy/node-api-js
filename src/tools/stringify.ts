...
type TGetLongKeys<T> = {
  [Key in keyof T]: T[Key] extends 'LONG'
    ? Key
...
type TFieldsToReplace =
  | TGetLongKeys<DataTransactionEntry<'LONG'>>
  | TGetLongKeys<ExchangeTransactionOrder<'LONG'>>
  | {
    ...
    }[keyof TransactionMap<'LONG'>] extends infer A
  ? A extends undefined
    ? never
    : A
  : never;
